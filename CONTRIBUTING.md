...
- Create a new `.yml` file in `app/languages/definitions/` and define the language properties.
  ...
  - `letterFrequency` _(optional)_ is the language's letters, most frequent first, as a single string (e.g. `etaoinshrdlucmfywgpbvkxqjz`). It ranks the letters offered when a key is pressed on its own, which on an ambiguous layout like the Compact QWERTY tile is the whole suggestion list — a lone key press can never match the dictionary. Without it, the order comes from `layout`, i.e. from the order the letters happen to be printed on the physical key. It does **not** affect ABC mode, the key hint, or which key types a letter. If present, it must cover every letter in `layout` and contain no duplicates; the build fails otherwise.
  ...
  - `ngramFile` _(optional)_ is the name of the n-gram file in `app/languages/dictionaries/` folder. Used for next-word predictions.
  ...
  - `supplementFile` _(optional)_ is the name of a second dictionary in `app/languages/dictionaries/`, merged into `dictionaryFile` at build time. Use it to add words without ever editing the base dictionary, so the base stays a clean upstream import. `en-contractions.csv` is the reference example.
  - `transparentChars` _(optional)_ lists characters that are kept in a word but skipped when generating its key sequence — making them reachable without a dedicated keystroke. For example, `transparentChars: '` lets "that's" be typed as `t-h-a-t-s`. Both encoders read this (the build-time one and the runtime one), so they cannot diverge. Keep the list to caseless characters only: the build matches after upper-casing and the runtime after lower-casing, so an alphabetic entry would make the two disagree.
